...
public class TaskPostVerifyId extends AsyncTask<String, Integer, String> {
    private String content;

    public TaskPostVerifyId(String content){
   ...
   //     String jsonUrl = "http://iems5722.albertauyeung.com/api/asgn2/send_message";
        String jsonUrl = "http://54.202.138.123:8000/pharos/api/verifyID";
        Log.i("POST", "doInBackground(Params... params) called");
        return postJsonData(jsonUrl);
    }

    protected void onPostExecute(String result) {
        super.onPostExecute(result);
        Log.i("POST", "onPostExecute(String result) called " + result);
        if (result != null)
        {
            asyncResponse.onDataReceivedSuccess(result);//将结果传给回调接口中的函数
        }
        else {
